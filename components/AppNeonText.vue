...
const update = async (newValue) => {
  ...
  data.maxHeight = Math.min(
    40,
    Math.max(textOneRef.value.offsetHeight, textTwoRef.value.offsetHeight)
  )
...
const rectAppearAttributes = computed(() => ({
  ...
  y: '0',
  width:
    data.widthAppear < data.widthDisappear
      ? Math.max(0, data.laserX - widthDifference.value / 2)
      : data.laserX,
  height: data.maxHeight,
}))
...
const rectDisappearAttributes = computed(() => ({
  ...
  width:
    data.widthAppear < data.widthDisappear
      ? Math.max(0, data.maxWidth - data.laserX)
      : Math.max(0, data.maxWidth - data.laserX),
  height: data.maxHeight,
}))
  ...
  <div class="app_neon_text" :style="boxStyle">
    ...
    <svg class="svg" v-bind="svgAttributes">
      <clipPath id="rect1">
        <rect
          v-bind="
            data.textOneActive ? rectDisappearAttributes : rectAppearAttributes
          "
        />
      </clipPath>
      <clipPath id="rect2">
        <rect
          v-bind="
            data.textOneActive ? rectAppearAttributes : rectDisappearAttributes
          "
        />
      </clipPath>
    </svg>
...
.app_neon_text {
  ...
  .text_two {
    ...
    text-shadow: +4px 4px 10px rgba(0, 90, 255, 0.66),
