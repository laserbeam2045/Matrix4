...
export default defineEventHandler(async (event) => {
  try {
    ...
    // const question = encodeURIComponent(getQuery(event).question as string).replace(" ", "%20")
    // const answer = encodeURIComponent(getQuery(event).answer as string).replace(" ", "%20")
