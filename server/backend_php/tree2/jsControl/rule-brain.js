...
function computerProcessing( app ) {			// computerが着手した時の処理
	...
	if( num == 2 && infoNum[num] == 9 ){
		...
		if (data.board[57] == SPACE) {			// 白がコウ立てを打った時
			...
			createLabel( "Ｃ", 4, 6, over, "black" );
		} else if (data.board[39] == SPACE) {
			data.turn = WHITE;
			computerPut( app, 39, 1000 );
			createLabel( "Ｃ", 4, 6, over, "black" );
			createLabel( "Ｄ", 3, 6, over, "white" );
		} else {
			lastPoint( app );
			createLabel( "Ｃ", 4, 6, over, "black" );
			createLabel( "Ｄ", 3, 6, over, "white" );
			...
			palette[2].style.visibility = "visible";
		}
	}
	...
	if( num == 3 && infoNum[num] == 3 ){
		...
		timer = window.setTimeout(function(){
			app.MainScene.Line.clear(5, 6);
			createLabel("Ｂ", 5, 6, over, "white");
		}, 1000);
	}
